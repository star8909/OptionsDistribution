...
def main():
    ...
    print(f"\n=== Term inversion (ratio > 1.X) 단독 (180d) ===")
    for r_t in [1.00, 1.05, 1.10, 1.15, 1.20, 1.30, 1.40]:
        sub = df[df["ratio"] >= r_t].dropna(subset=["future_180d"])
        if len(sub) < 10:
            continue
        ...
        print(f"  ratio>{r_t:.2f}: N={len(sub):>4} 180d {m:>+7.2f}% Win {w:>5.1f}% Sharpe {s:>5.2f} {marker}")

    print(f"\n=== Term inversion + DD (180d) ===")
    combos = [
    ...
    for r_t, dd_t in combos:
        sub = df[(df["ratio"] >= r_t) & (df["dd"] <= dd_t)].dropna(subset=["future_180d"])
        if len(sub) < 5:
            continue
        m = sub["future_180d"].mean() * 100
        w = (sub["future_180d"] > 0).sum() / len(sub) * 100
        s = sub["future_180d"].mean() / sub["future_180d"].std() * np.sqrt(2) if sub["future_180d"].std() > 0 else 0
        marker = "🚀" if w >= 95 and m > 15 else ""
        print(f"  ratio>{r_t} DD<{int(dd_t*100)}%: N={len(sub):>3} 180d {m:>+7.2f}% Win {w:>5.1f}% Sharpe {s:>5.2f} {marker}")

    print(f"\n=== Term inversion + DD (21d) ===")
    for r_t, dd_t in combos:
        sub = df[(df["ratio"] >= r_t) & (df["dd"] <= dd_t)].dropna(subset=["future_21d"])
        if len(sub) < 5:
            continue
        m = sub["future_21d"].mean() * 100
        w = (sub["future_21d"] > 0).sum() / len(sub) * 100
        marker = "🚀" if w >= 95 and m > 5 else ""
        print(f"  ratio>{r_t} DD<{int(dd_t*100)}%: N={len(sub):>3} 21d {m:>+6.2f}% Win {w:>5.1f}% {marker}")
