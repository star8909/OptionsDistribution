...
def main():
    ...
    for v_t, vv_t, term_t, dd_t in combos:
        sub = df[(df["vix"] >= v_t) & (df["vvix"] >= vv_t) & (df["term"] >= term_t) & (df["dd"] <= dd_t)].dropna(subset=["future_180d"])
        if len(sub) < 3:
            continue
        ...
        print(f"  V>{v_t} VV>{vv_t} term>{term_t} DD<{int(dd_t*100)}%: N={len(sub):>3} 180d {m:>+7.2f}% Win {w:>5.1f}% Sh {s:>5.2f} {marker}")

    print(f"\n=== ULTRA panic (21d) ===")
    for v_t, vv_t, term_t, dd_t in combos:
        sub = df[(df["vix"] >= v_t) & (df["vvix"] >= vv_t) & (df["term"] >= term_t) & (df["dd"] <= dd_t)].dropna(subset=["future_21d"])
        if len(sub) < 3:
            continue
        m = sub["future_21d"].mean() * 100
        w = (sub["future_21d"] > 0).sum() / len(sub) * 100
        marker = "🚀" if w >= 95 and m > 5 else ""
        print(f"  V>{v_t} VV>{vv_t} term>{term_t} DD<{int(dd_t*100)}%: N={len(sub):>3} 21d {m:>+6.2f}% Win {w:>5.1f}% {marker}")
