...
def main():
    ...
    print(f"\n=== VIX Threshold 종합 (180d future) ===")
    print(f"  {'Threshold':15s} {'N':>5} {'Mean':>8} {'Win%':>6} {'Sharpe':>7}")
    for thr in [15, 18, 20, 22, 25, 27, 30, 35, 40, 50]:
        sub = df[df["vix"] >= thr].dropna(subset=["future_180d"])
        if len(sub) < 30:
            continue
        ...
        print(f"  VIX > {thr}     {len(sub):>5} {m:>+7.2f}% {w:>5.1f}% {s:>6.2f}  {marker}")

    print(f"\n=== VIX 낮은 threshold 후 SPY (over-confidence) ===")
    print(f"  {'Threshold':15s} {'N':>5} {'180d mean':>10} {'Win%':>6}")
    for thr in [10, 12, 14, 16]:
        ...
        print(f"  VIX < {thr}     {len(sub):>5} {m:>+9.2f}% {w:>5.1f}%  {marker}")
